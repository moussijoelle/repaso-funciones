...
const esPar = (numero) => {                                    // entrada: number | salida: boolean
    return numero % 2 === 0;                                   // true si el resto de dividir entre 2 es 0
};

const esImpar = (numero) => {                                  // entrada: number | salida: boolean
    return numero % 2 !== 0;                                   // true si NO es divisible entre 2
};

const miFuncion = (array) => {                                 // entrada: array | salida: object
    const pares = array.filter(esPar);                         // guarda solo los pares
    const impares = array.filter(esImpar);                     // guarda solo los impares
    return { pares, impares };                                 // devuelve { pares: [...], impares: [...] }
};
...
const formatearParesImpares = (objeto) => {                    // entrada: object | salida: string
    const textoPares = `[${objeto.pares.join(", ")}]`;         // convierte pares a texto legible
    const textoImpares = `[${objeto.impares.join(", ")}]`;     // convierte impares a texto legible
    return `Pares: ${textoPares} | Impares: ${textoImpares}`;  // une todo para pantalla
};

const mensajeResultado = (esValido, resultado) => {            // entrada: boolean + object | salida: string
    const mensajeError = "Usa comas entre enteros, no puntos (ej: 1, 2, 3)."; // texto si falla
    const mensajeOk = formatearParesImpares(resultado);        // texto si todo va bien
...
const funcionPrincipal = (evento) => {                         // entrada: event | salida: ninguna
    ...
    const resultado = esValido ? miFuncion(valores) : { pares: [], impares: [] }; // calcula si es válido
